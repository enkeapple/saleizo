...
2. **Verify each against the repo this session.** The repo's manifest (`package.json`/`Cargo.toml`/`go.mod`/…)/Makefile/CI for commands; the filesystem for paths/folders; the lockfile for versions; the skill/command registry for routing targets. Do not stop at the obvious one — the unrunnable checklist row is usually the one you'd skim. Then apply, to every generated slot in both files, the eight **manual-governance drift classes** in [references/drift-classes.md](./references/drift-classes.md) and the **placeholder-key** detection contract in [placeholder-keys.md](../shared/placeholder-keys.md). Each reference carries its own falsifiable signals — read them per signal, don't eyeball. The three fidelity classes (STRICT-block drift, path-noise, cross-app contamination) compare the file against the canonical templates in `bootstrapping-claude-md`'s `assets/`.
3. **Run the cross-file consistency pass.** The root and the manual must agree: same pipeline name/vocabulary, same Role/position **verbatim** (the root's "Engineering system" persona must equal the manual's `## Role` `<POSITION>`), same commands, no contradiction. Confirm every `src/**` anchor path lives once in the manual's `## Key files` table (path-noise class), and that each [STRICT] block matches the canonical template (STRICT-block drift class). The root stays a thin index — governance that is duplicated in both files is drift waiting to happen; it should live in the manual and be pointed to.
...
| "The manual's lessons/design-docs/baseline wording is close enough." | The eight manual-governance drift classes each misfire silently; apply [references/drift-classes.md](./references/drift-classes.md) per signal, don't eyeball them — including a baseline section that's absent only when the repo *declared* none. |
| "The two sibling repos read a bit differently, but each is internally fine." | A [STRICT] block differing between siblings, or one app's manual carrying another app's symbols, is drift (STRICT-block / cross-app-contamination classes) — compare against the canonical template, not just within one file. |
