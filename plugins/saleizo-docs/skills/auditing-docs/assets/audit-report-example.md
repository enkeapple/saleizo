...
Manifest: `.claude/docs-manifest.json` (v1, docsRoot `docs/`). 7 findings.
...
| 6 | `missing` | `docs/architecture.md` | manifest entry present, `doc` file absent on disk | `scaffold` \| `remove-entry` |
| 7 | `marker-malformed` | `docs/dupe.md` | two blocks share `block="scripts"` | `repair` |
...
```text
Audit found 7 issues, each with a recommended disposition (table above).
  1. Apply recommended   → regenerate #1, refresh #2, delegate #3, treat-as-prose #4, adopt #5, scaffold #6, repair #7
