...
Add a new doc-type **contract** to a project. A contract is a **data file** — a derivation spec, not a `SKILL.md` — so `writing-skills`' frontmatter validators (frontmatter ≤1024, name regex) do NOT apply to the contract file. A contract's tests are two, and both must pass before it ships: a **schema-valid header** and a **scaffold→audit round-trip** that proves the render is idempotent. Schema and marker grammar are single-sourced in [../shared/manifest-schema.md](../shared/manifest-schema.md); the step list is [references/authoring-a-contract.md](references/authoring-a-contract.md).
...
- Declaring the round-trip passed from reasoning instead of actually running `scaffolding-docs` then `auditing-docs` and observing CLEAN-then-one-finding (step 5).
...
| Excuse | Reality |
| --- | --- |
| "The render is obviously deterministic — no need to render twice." | Reasoning is not the test. Render twice and observe byte-identical output (step 4). |
| "The audit would report clean, so the round-trip passes." | A narrated round-trip is not a run — actually invoke `scaffolding-docs` then `auditing-docs` and read the verdicts (step 5). |
| "A round-trip is overkill for a one-column table." | Ship only after an observed round-trip: CLEAN on unchanged, exactly one finding on change (step 5). |
| "A free-text summary column is fine in a machine block." | It is not losslessly re-derivable → perpetual drift; move it to hybrid prose (step 3). |
