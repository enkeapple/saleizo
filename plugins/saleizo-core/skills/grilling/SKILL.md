...
> **Hard gate:** Do NOT write code, scaffold, split tickets, or invoke an implementation/planning skill until the design is shared AND the user approves it. This holds for every task that reaches grilling, however simple it seems — whether a small, well-understood change skips design and routes straight to execution is the front door's (`sdd-lifecycle`) classification call, not a judgment made from inside grilling. Once a task is here, "too simple to design" is the rationalization that wastes the most work — the design can be three sentences, but it gets presented and approved.
