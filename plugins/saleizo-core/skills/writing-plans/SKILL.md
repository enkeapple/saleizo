...
For anything beyond a small plan — **beyond small** = more than ~2 tasks/files, a shared contract, or cross-task coupling; a single-task, single-file plan is *small* — dispatch an **independent reviewer: a fresh subagent with zero shared context, handed the spec** — using [assets/plan-reviewer-prompt.md](./assets/plan-reviewer-prompt.md). Its remit is the author-blind class, **not** a re-run of the self-review scan: it re-derives spec coverage from the spec itself (you *believe* you covered it — it verifies), judges zero-context buildability (could a context-free engineer build this out of order without guessing — an absence you hold too much context to feel), and reviews the **design quality of the code** the plan will have written verbatim (it invokes `codebase-design` for the deep-module lens — the author's own taste is a blind spot, and the plan's code blocks ship to execution as written). The asset carries the full checklist. Fix issues and re-review.
