...
2. **Scan every layer the change touches** and classify each NONE / PARTIAL / FULL. The layers of a skill change are: `SKILL.md` frontmatter → `SKILL.md` body → `references/*.md` → `skills-routing.json` (triggers) → `.claude/hooks/*.sh`. A new/renamed/deleted skill or a trigger change is NOT done until routing matches disk and the flat symlink exists (see [skill-routing-sync.md](./skill-routing-sync.md)).
