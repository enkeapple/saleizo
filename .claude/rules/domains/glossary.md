# Glossary — what the SDD-framework words mean here (and what "test" is NOT)
...
- **"this repo" vs "consumer repo" / "target repo" / "the app"**.
...
| 2 | **rule** | `.claude/rules/<area>/*.md` (`domains/` = glossary + framework; `verification/` `authoring/` `workflow/` `conduct/` = cross-cutting concerns; `clean-code/` `anti-patterns/` = code quality; `flbco/` = repo-local config) | loaded on demand, never auto-injected | a convention/process doc the agent reads when relevant |
| 3 | **hook** | routing/metric/session/quality hooks ship in the `saleizo-controls` plugin (`plugins/saleizo-controls/hooks/`), wired by its `hooks.json`; guard hooks remain repo-local in `hooks/guards/`, surfaced via `.claude/hooks/` symlinks, wired by root `settings.json` | runs on tool events | a gate/logger: routing (`detect-bypass`, `skill-gate`, `log-skill-usage`), metric (`friction-log`), guards (`security-guard`, `bash-read-guard`, `read-guard`, `edit-write-guard`), quality (`quality`), notification (`notify`), session (`reset-turn-budget`, `token-guard`, `lessons-nudge`, `handoff-nudge`, `session-telemetry-digest`); authored/edited test-first via `writing-hooks` (fixture-execution is its RED/GREEN) |
...
- **"test"** — resolve by mode. In **AUTHOR/AUDIT** (working on this repo's own skills) → a **subagent pressure scenario**: RED = run the scenario WITHOUT the skill and watch it fail; GREEN = re-run WITH the skill and confirm compliance. In **APPLY** (the `test-driven-development` skill on a consumer repo) → a real automated unit test on that repo's stack. Never a unit test *of this repo* — there is none.
...
- **"this repo"** — always this repo: the skills are the *product* (the Saleizo marketplace). **"consumer / target repo" / "the app"** — a separate codebase the chain is APPLIED to; it supplies the stack, paths, and commands the agnostic skills never bake in.
...
- An earlier version of `framework.md` (before it was de-leaked and moved from `common/` into `domains/`) described a TypeScript/React-Native + `pnpm` app — that was a **project leak**, not this repo. The current `framework.md` is the agnostic Charter; never reintroduce stack-specific verification (`pnpm test`, Vitest) as if it were this repo's.
...
- Did not attribute any `pnpm` / build / unit-test command to this repo itself.
