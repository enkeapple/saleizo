...
- **Export-floor policy rule.** On a strong agent this largely reproduces no RED (intent-revealing naming is default-good behavior — the canonical no-op of [scoping-rule-value](../common/scoping-rule-value.md)); its always-on load is earned in weaker / non-agentic consumer harnesses. The predicate-prefix (`is`/`has`) and no-type-suffix parts steer even a capable model; the generic-placeholder ban is the policy half.
- Function *name quality* is shared with [concise-functions](./concise-functions.md), which owns *splitting* a multi-task function; this rule owns whether a name reveals purpose — cross-link, not restate.
