...
| # | Item | Done when |
| --- | --- | --- |
| 1 | RED observed (by tier) | **Behavioral** edit: the baseline run failed as expected (or `[N/A]` — control showed no failure); for an **export-bound** skill (value is for weaker/non-agentic consumer harnesses), a green in-repo RED across tiers is NOT an `[N/A]`-cut — RED against a representative export floor, or ship on the policy basis and record which (see-also `rules/authoring/fair-red-baseline.md`, `rules/authoring/scoping-skill-value.md`, `rules/authoring/scoping-rule-value.md`). **Descriptive** edit → `[N/A — descriptive]` + one cited real prior failure in the commit; **Mechanical** edit → `[N/A — mechanical]`, validators only. Classify via the reversion test ([framework.md](./rules/domains/framework.md) → Iron Law) |
