...
- A **manual-governance drift signal** fires — bypass wording, an over-broad capture criterion, an append-only lessons model, or a missing/multi-valued design-docs home — and you let it pass; each is drift in a chain repo (signals + correct forms in [references/drift-classes.md](./references/drift-classes.md)).
...
| "The manual's lessons/design-docs wording is close enough." | The four manual-governance drift classes (bypass, over-broad capture, append-only, multi-valued design-docs) each misfire silently; apply [references/drift-classes.md](./references/drift-classes.md) per signal, don't eyeball them. |
